...
export default async function handler(req, res) {
  ...
  if (promoError || !promo) return res.status(404).json({ error: 'Промокод не найден или уже недействителен' });
  if (promo.used_count >= promo.max_uses) return res.status(400).json({ error: 'Промокод уже использован максимальное количество раз' });
